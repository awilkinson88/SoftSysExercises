...
Value *make_int_value(int i) fac
{
...
typedef struct {
    void *key;
    int (*hash) (void *);
    int (*equal) (void *, void *); //Equal is an equal function
} Hashable;
...
{
    if (h1->equal != h2->equal) return 0; //checks to see if the hash method is the same.
    else return h1->equal(h1->key, h2->key); //compares keys
}
...
Value *list_lookup(Node *list, Hashable *key)
{
    Node *current = list;
    while (current !=NULL){
        if (equal_hashable(current->key, key)) return current->value;
...
Map *make_map(int n)
//Had to refer to the solutions for this one, but I understand everything
{
    int i;
    Map *map = (Map *) malloc(sizeof(Map));
    map->n = n;
    map->lists = (Node **) malloc(sizeof (Node *) * n); //allocates space for each node list, times number of lists
    for (i = 0; i<n; i++){
        map->lists[i] = NULL; //initializes each node list to null
    }
    return map;
}
...
{
    int hashval = hash_hashable(key) % map->n; //splits it into 'bins'
    Node *list = map->lists[hashval]; //finds the correct 'bin'
    map->lists[hashval] = prepend(key,value,list); //adds keyval pair to start of bin
}
...
{
    int hashval = hash_hashable(key) % map->n; //finds hashval
